...
class MoMPipeline(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, embedding_dim: int, num_memories: int, k: int, output_activation: nn.Module = nn.Identity(), mom_implementation : nn.Module = None, update_module : nn.Module = None, *args, **kwargs):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mom_out, _, aux_loss = self.mom(x_emb, M_0)
